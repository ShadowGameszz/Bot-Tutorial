...
module.exports = (client) => {
    readdirSync(join("src/comandos")).forEach(dir => { //puxa todos diretórios dos cmds
        ...
        for (let file of commands) { // faz um loop
            let pull = require(`../src/comandos/${dir}/${file}`); //Pega o comando
    
            if (pull.name) {//se ele consiga pegar o nome do comando ele seta o comando no client
                client.commands.set(pull.name, pull);
                console.log(`[COMMANDS] - ${file} Carregado com sucesso`)
            } else {
